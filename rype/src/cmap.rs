...
impl Subtable {
    fn gid_for_cp(&self, cp: u32) -> Result<u32> {
        match self {
            Subtable::Format4(data) => Self::gid_for_cp_format_4(data, cp),
            Subtable::Format12(data) => Self::gid_for_cp_format_12(data, cp),
        }
    }
    ...
    fn gid_for_cp_format_4(data: &[u8], cp: u32) -> Result<u32> {
        ...
        for offset in (0..segcount_x2).step_by(2) {
            ...
            return get_u16(data, glyph_off).map(|u| u as u32);
        }
        Ok(0)
    }
