...
impl FontSource {
    pub(super) fn new() -> FontSource {
        let factory = Factory::new().expect("Failed to create directwrite factory");
        let font_collection = FontCollection::system_font_collection(&factory, false)
            .expect("Failed to get directwrite font collection");
        FontSource {
            factory: factory,
